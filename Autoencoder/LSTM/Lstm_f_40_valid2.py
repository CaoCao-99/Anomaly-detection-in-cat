...
# Limping
cat_normal_path = "/home/sm32289/Cat_Pose/KeypointDetection(Yolo)/predict/Normal/keypoints"
...
# Throwing-up
cat_throwingup_path = "/home/sm32289/Cat_Pose/KeypointDetection(Yolo)/predict/throwing-up-refined/keypoints"
...
# Sneezing
cat_sneezing_path = "/home/sm32289/Cat_Pose/KeypointDetection(Yolo)/predict/Cat_Sneezing/keypoints"
...
# Limping
cat_limping_path = "/home/sm32289/Cat_Pose/KeypointDetection(Yolo)/predict/cat_limping_refined/keypoints"
...
checkpoint_filepath = os.path.join(save_model_dir, 'best_model.h5')
...
# Encoder
autoencoder.add(tf.keras.layers.LSTM(64, activation='relu', input_shape=(sequence_length, num_features)))
autoencoder.add(tf.keras.layers.LSTM(32, activation='relu'))
autoencoder.add(tf.keras.layers.LSTM(16, activation='relu'))
autoencoder.add(tf.keras.layers.LSTM(8, activation='relu')) #8개로 줄이긴
# Decoder
autoencoder.add(tf.keras.layers.LSTM(16, activation='relu'))
autoencoder.add(tf.keras.layers.LSTM(32, activation='relu'))
autoencoder.add(tf.keras.layers.LSTM(64, activation='relu'))
autoencoder.add(tf.keras.layers.LSTM(num_features, activation="sigmoid"))
...
history = autoencoder.fit(x_train_y0, x_train_y0,
                     ...
                     validation_data=(x_valid_y0, x_valid_y0) , callbacks=[early_stopping, model_checkpoint])

autoencoder2 = models.Sequential()
# Encoder
autoencoder2.add(tf.keras.layers.LSTM(64, activation='relu', input_shape=(sequence_length, num_features)))
autoencoder2.add(tf.keras.layers.LSTM(32, activation='relu'))
autoencoder2.add(tf.keras.layers.LSTM(16, activation='relu'))
autoencoder2.add(tf.keras.layers.LSTM(8, activation='relu')) #8개로 줄이긴
# Decoder
autoencoder2.add(tf.keras.layers.LSTM(16, activation='relu'))
autoencoder2.add(tf.keras.layers.LSTM(32, activation='relu'))
autoencoder2.add(tf.keras.layers.LSTM(64, activation='relu'))
autoencoder2.add(tf.keras.layers.LSTM(num_features, activation="sigmoid"))
...
# compile
autoencoder2.compile(loss='mse', optimizer=optimizers.Adam(lr),metrics=["acc"])
# print(x_train_y0)
# print(x_valid_y0)

# checkpoint_filepath2 = os.path.join(save_model_dir, 'best_model.h5')


# fit
history2 = autoencoder2.fit(x_train_y0, x_train_y0,
                     epochs=epochs, batch_size=batch,
                     validation_data=(x_valid_y1, x_valid_y1) , callbacks=[early_stopping])


history_save_dir = os.path.join(save_model_dir,'history.txt')
history2_save_dir = os.path.join(save_model_dir,'history2.txt')
...
with open(history2_save_dir, 'a') as file:
    file.write(str(history2.history))


plt.plot(history.history['loss'], label='train loss')
plt.plot(history.history['val_loss'], label='valid loss1')
plt.plot(history2.history['val_loss'], label='valid loss2')
